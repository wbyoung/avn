...
assertEqual "after-cd --color ${__testdir}/examples/v0.11 ../v0.11" "${__written}" || exit 1

# change to a directory where the before hook will be called
echo "" > ${__tmp} # clear output
cd "../../examples/home"
__written=`echo $(cat ${__tmp})`
assertEqual "before-cd --color ${__testdir}/examples/v0.11 ../../examples/home" "${__written}" || exit 1
