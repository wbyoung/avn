...
var shellInitScriptChooser = function() {
  ...
  .then(function(fileExistenceValues) {
    var bashrcExists = fileExistenceValues[0];
    var bashProfileExists = fileExistenceValues[1];
    ...
    else if (bashrcExists || bashProfileExists) { // XOR file existence
      return (bashrcExists && bashrcFileName) ||
             (bashProfileExists && bashProfileFileName);
    }
...
module.exports.update = function() {
  ...
  .then(function(initScript) {
    ...
    if (appendFile === bashProfileFileName) {
      Promise.map([bashProfileFileName, '.zshrc'], function(name) {
