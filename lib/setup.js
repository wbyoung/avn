...
profile.append = function(name, options) {
  var opts = options || {};
  var result;
  var line = profile.script();
  ...
  if (contents.match(/avn\.sh/)) {
    ...
    result = {};
    result.type = 'change';
    result.promise = write(file, line, { flags: 'a' })
    .then(function() {
...
profile.update = function() {
  ...
  ['.bash_profile', '.zshrc'].forEach(function(name) {
    var append = profile.append(name);
    if (append) {
  ...
  if (!handled) {
    var append = profile.append('.bash_profile', { force: true });
    promise = promise.then(function() { return append.promise; });
    handled = true;
    changed = changed || (append.type === 'change');
  }
